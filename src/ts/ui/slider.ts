...
export class Slider extends Graphics {
  ...
  constructor(
    ...
    if (labelOffset != undefined) {
      this.label = this.scene.add.text(this.x + labelOffset.x, this.y + labelOffset.y, undefinedText, {
        color: '#ffffff',
        fontSize: '24px'
      })
    }
  ...
  private doDrag(pointer: Pointer) {
    ...
    this.value = Phaser.Math.Clamp(
      (pointer.x - this.x - this.handle.width / 2) / (this.width - this.handle.width),
      0,
      1
    )
    this.updateHandle()
