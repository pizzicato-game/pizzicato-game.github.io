import { PlayableTrackLayerData, SongTimePosition, TrackData, TrackLayerData } from '../level/trackTypes'
...
export class Track {
  ...
  public setBPM(bpmIndex: number) {
    ...
    const ticksPerBeat: number = ticksPerBar / this.data.timeSignatureDenominator
  ...
  // Preloads all the notes in levelDir/<trackKey>/<layer_id>/<soundKey>.ogg
  public preloadNotes(scene: HandScene) {
    ...
    this.forEachLayer(undefined, (layer: PlayableTrackLayerData) => {
      ...
      l.nodes.forEach((node, _nodeIndex) => {
        ...
        const soundPath: string = this.trackDir + layer.id + '/' + node.soundKey + soundFileExtension
  ...
  private preloadLoops(scene: HandScene) {
    this.forEachLayer((layer: TrackLayerData) => {
      ...
      layer.soundLoopKeys.forEach(loopKey => {
        ...
        const soundPath: string = this.trackDir + layer.id + '/' + loopKey + soundFileExtension
  ...
  public forEachLayer(
    ...
    playableLayerCallback?: (playableLayer: PlayableTrackLayerData, index: number) => void
  ) {
  ...
  public addBackgroundAudioTracks(
    ...
    this.forEachLayer((layer: TrackLayerData, index: number) => {
      if (predicate == undefined || predicate(layer, index)) {
        assert(layer.soundLoopKeys !== undefined)
        assert(bpmIndex < layer.soundLoopKeys.length, 'BPM index out of range of layer sound loop keys')
        audioTracks.push(scene.sound.add(this.getSoundKey(layer, layer.soundLoopKeys[bpmIndex]), soundConfig))
      }
    })
    return audioTracks
  }
  ...
  timeToSongTimePosition(time: number): SongTimePosition {
    ...
    const step = Math.floor(beatInBar * (barLength / this.data.timeSignatureDenominator) * barLength) + 1
  ...
  songTimePositionToTime(position: SongTimePosition): number {
    ...
    const beats =
      ...
      (position.step - 1) / (barLength * (barLength / this.data.timeSignatureDenominator)) +
  ...
  private sortNodes() {
    this.forEachLayer(undefined, (layer: PlayableTrackLayerData) => {
      layer.nodes.sort((a, b) => {
        // Compare bars
        if (a.timePosition.bar !== b.timePosition.bar) return a.timePosition.bar - b.timePosition.bar
        // Bars are equal, compare steps
        if (a.timePosition.step !== b.timePosition.step) return a.timePosition.step - b.timePosition.step
