...
export default class Level {
  ...
  public setBPM(bpmIndex: number) {
    assert(bpmIndex != invalidBpmIndex, 'BPM must be set before starting the level')
  ...
  private preloadLayers() {
    ...
    this.track.forEachLayer(undefined, (layer: PlayableTrackLayerData, _layerIndex: number) => {
  ...
  public addBackgroundAudio(
    ...
    assert(this.bpmIndex_ != invalidBpmIndex, 'Set BPM before adding background audio to level')
    this.removeBackgroundAudio(scene)
    this.activeAudioTracks = this.track.addBackgroundAudioTracks(scene, soundConfig, this.bpmIndex_, predicate)
  }
  ...
  private startLayer() {
    ...
    this.score.resetLayerScores(layer.data.nodes.length, layer.data.progressCount)
