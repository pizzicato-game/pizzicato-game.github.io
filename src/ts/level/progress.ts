...
import { entireLayerProgressRequired, progressBarDepth, progressBarOptions, startingProgress } from '../core/config'
...
export default class Progress extends Group {
  ...
  public preload() {
    this.scene.load.image(progressBarOptions.key, absPath(progressBarOptions.path))
  }

  public setup(initialProgress: number, nodeCount: number, requiredProgress: number, tints: number[]) {
    assert(requiredProgress <= nodeCount)
    assert(tints.length === nodeCount, 'Must pass tints equal in number to nodes')
    assert(nodeCount > 0)
    ...
    const progressSpriteWidth: number = progressBarOptions.size.x / this.nodeCount
    ...
    this.createMultiple({
      ...
      visible: true,
      setXY: {
        x: progressBarOptions.position.x - progressBarOptions.size.x / 2 + progressSpriteWidth / 2,
  ...
  public redraw() {
    if (this.current <= this.nodeCount) {
      ...
      const required: number =
        this.requiredProgress === entireLayerProgressRequired ? this.nodeCount : this.requiredProgress - 1
  ...
  public canProgress(): boolean {
    const perfectCompletionRequired: boolean = this.requiredProgress === entireLayerProgressRequired
    return (
