...
export default class Scoreboard extends HandScene {
  ...
  preload() {
    ...
    this.load.image(layerScoreBackgroundTextureKey, absPath(layerScoreBackgroundPath))
    this.load.image(scoreboardBackgroundTextureKey, absPath(scoreboardBackgroundPath))
  ...
  create() {
    ...
    if (!config.autoSaveCSV) {
      this.saveButton = new Button(
        this,
        new Vector2(horizontalCenter - windowWidth * leftButtonOffset, windowHeight * buttonBottomLevel),
        new Vector2(0.6, 0.6),
        saveCSVButtonTextureKey,
        buttonPinchSoundKey,
        true
    ...
    const totalNotes = this.levelStats.layersStats.reduce((sum, x) => sum + x.total, 0)
    const hitNotes = this.levelStats.layersStats.reduce((sum, x) => sum + x.correct, 0)
    const missedNotes = this.levelStats.layersStats.reduce((sum, x) => sum + x.miss, 0)
    const earlyNotes = this.levelStats.layersStats.reduce((sum, x) => sum + x.early, 0)
    const lateNotes = this.levelStats.layersStats.reduce((sum, x) => sum + x.late, 0)
    ...
    infoText.setPosition(horizontalCenter - infoText.width / 2, windowHeight * 0.19)

    //------------------------------------------
    const layerBackgroundOffsets = [0.2 + 0.05, 0.5, 0.8 - 0.05]
    const layerTextOffsets = layerBackgroundOffsets
    this.level.track.forEachLayer(undefined, (layer: PlayableTrackLayerData, i: number) => {
      this.add
        .sprite(0, 0, layerScoreBackgroundTextureKey)
        .setOrigin(0.5, 0.5)
        .setScale(1.35 * scaleFactor, 1.15 * scaleFactor)
        .setAlpha(scoreboardBackgroundOpacity)
        .setPosition(windowWidth * layerBackgroundOffsets[i], windowHeight * 0.63)
      ...
      layerTitle.setPosition(windowWidth * layerTextOffsets[i] - layerTitle.width / 2, windowHeight * 0.53)
      ...
      layerText.setPosition(windowWidth * layerTextOffsets[i] - layerText.width / 2, windowHeight * 0.575 + 0.01)
    })
