...
export default class LevelSelect extends HandScene {
  ...
  constructor() {
    ...
    assert(this.currentLevelIndex >= 0, 'Level select default level must be >= 0')
  }
  ...
  preload() {
    ...
    this.load.image(previewVideoBackgroundTextureKey, absPath(previewVideoBackgroundTexturePath))
    this.load.image(songNameBackgroundTextureKey, absPath(songNameBackgroundTexturePath))
  }
  ...
  create() {
    ...
    this.play = new Button(
      this,
      new Vector2(horizontalCenter, levelSelectButtonBottomLevel * windowHeight),
      standardButtonScale,
      playButtonTextureKey,
      buttonPinchSoundKey,
      true
    ...
    this.previousLevel = new Button(
      this,
      new Vector2(horizontalCenter - windowWidth * levelSelectButtonGap, levelSelectbuttonTopLevel * windowHeight),
      new Vector2(0.4, 0.4),
      leftArrowTextureKey,
      buttonPinchSoundKey,
      true
    ...
    const _song: Sprite = this.add
      .sprite(horizontalCenter, windowHeight * levelSelectbuttonTopLevel, songNameBackgroundTextureKey)
      .setTintFill(0x000000)
      .setScale(0.4, 0.4)
      .setOrigin(0.5, 0.5)

    this.songName = this.add
      .text(horizontalCenter, windowHeight * levelSelectbuttonTopLevel, undefinedText, {
  ...
  private createDifficultyButtons(level: Level) {
    ...
    const buttonKeys: string[] = [easyButtonTextureKey, mediumButtonTextureKey, hardButtonTextureKey]

    const positions: Vector2[] = [
      new Vector2(horizontalCenter - windowWidth * levelSelectLevelButtonGap, levelSelectButtonMidLevel * windowHeight),
      new Vector2(horizontalCenter, levelSelectButtonMidLevel * windowHeight),
      new Vector2(horizontalCenter + windowWidth * levelSelectLevelButtonGap, levelSelectButtonMidLevel * windowHeight)
    ]

    assert(buttonKeys.length >= bpms.length, 'buttonKeys array length must at least match the number of track bpms')
    assert(positions.length >= bpms.length, 'positions array length must at least match the number of track bpms')
    ...
    for (let i = 0; i < bpms.length; i++) {
      ...
      const isDefaultDifficultyButton: boolean = i == defaultDifficultyButtonIndex
  ...
  private startVideo(level: Level) {
    ...
    if (level.hasPreviewVideo()) {
      ...
      const speedMultiplier = level.track.data.bpm[level.bpmIndex] / level.track.data.bpm[0]
  ...
  private cycleLevel(amount: number) {
    ...
    this.currentLevelIndex = (((this.currentLevelIndex + amount) % levelCount) + levelCount) % levelCount
  }

  private getCurrentLevel(): Level {
    assert(this.currentLevelIndex < this.levels.length, 'Current level index out of range of this.levels array')
