...
import { HandIndex, handLandmarkCount, HandLandmarkIndex } from '../objects/handLandmarks'
import { landmarkDetectionOptions, modelPath, wasmDirectory } from '../core/config'
...
export class HandTracker {
  ...
  public async init(progressCallback: (text: string) => void): Promise<boolean | string | string[]> {
    ...
    return new Promise<boolean | string | string[]>(async (resolve, reject) => {
      ...
      await HandLandmarker.createFromOptions(wasmFileset, landmarkDetectionOptions)
        .then(result => (this.handLandmarker_ = result))
        .catch(err => {
          if (err instanceof Event && (err as Event).type == 'error')
            reject('Failed to fetch vision_wasm_internal.js in specified wasm directory')
          if (err instanceof TypeError) reject('Failed to fetch hand_landmark.task file')
          reject(err)
          return
        })
  ...
  public update() {
    ...
    this.results_ = this.handLandmarker.detectForVideo(webcam.video, performance.now())
  }
  ...
  public landmarksFound(handIndex: HandIndex = 0): boolean {
    return this.results != undefined && handIndex < this.results.landmarks.length
  }

  public landmarkFound(landmarkIndex: HandLandmarkIndex, handIndex: HandIndex = 0): boolean {
  ...
  public getLandmarkPosition(landmarkIndex: HandLandmarkIndex, handIndex: HandIndex = 0): Vector2 | undefined {
    if (!this.landmarksFound(handIndex)) return undefined

    const landmark = this.results.landmarks[handIndex][landmarkIndex]
    const position = normalizedToWindow(new Vector2(landmark.x, landmark.y))
    return new Vector2(window.innerWidth - position.x, position.y) /* mirror landmarks in the x-direction */
  }

  public forEachLandmarkPosition(landmarkPositionCallback: (position: Vector2) => void, handIndex: HandIndex = 0) {
    for (let i = 0; i < handLandmarkCount; ++i) {
      const position: Vector2 | undefined = this.getLandmarkPosition(i, handIndex)
