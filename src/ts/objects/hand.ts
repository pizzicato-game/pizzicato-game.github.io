...
import { GameObject, MatterOverlapData, Scene, Vector2 } from '../core/phaserTypes'
import {
  fingerSpriteKey,
  fingerSpriteScale,
  focalLength,
...
export class Hand extends GameObject {
  ...
  private isObserver(observer: Observer): boolean {
    return (
      this.observers_.find((o: Observer) => {
        return o.fingerName === observer.fingerName && o.object == observer.object
      }) !== undefined
    )
  }
  ...
  private setObserverHover(observer: Observer, state: boolean) {
    ...
    assert(index !== invalidFingerIndex, 'Cannot set observer hover before observer is pushed')
  ...
  constructor(scene: Scene) {
    ...
    const _index: Finger = addFinger(indexFingerId, HandLandmarkIndex.INDEX_FINGER_TIP)
    const _middle: Finger = addFinger(middleFingerId, HandLandmarkIndex.MIDDLE_FINGER_TIP)
    const _ring: Finger = addFinger(ringFingerId, HandLandmarkIndex.RING_FINGER_TIP)
    const _pinky: Finger = addFinger(pinkyFingerId, HandLandmarkIndex.PINKY_TIP)

    // TODO: Possibly break this up into smaller functions.
    const addOverlapEmitter = (
      onOverlap: (overlapCallback: (overlap: MatterOverlapData) => void) => void,
        ...
        if (finger !== undefined) {
          for (const observer of this.observers) {
            if (
              ...
              observer.object.emit(finger.name + FingerOverlapEvents.pinch + state)
            }
          }
          ...
          for (const f of this.fingers) {
            ...
            const fingerOverlapping: boolean = this.scene.matter.overlap(overlap.bodyB, [f])
            if (state === FingerOverlapStates.start && fingerOverlapping) {
              obj.emit(f.name + FingerOverlapEvents.hover + FingerOverlapStates.start)
              ...
              if (fingerOverlapping && wasHovering) {
                obj.emit(f.name + FingerOverlapEvents.hover + FingerOverlapStates.continue)
                this.setObserverHover(o, true)
              } else if (fingerOverlapping && !wasHovering) {
                obj.emit(f.name + FingerOverlapEvents.hover + FingerOverlapStates.start)
                this.setObserverHover(o, true)
              } else if (!fingerOverlapping && wasHovering) {
                obj.emit(f.name + FingerOverlapEvents.hover + FingerOverlapStates.end)
                this.setObserverHover(o, false)
              }
            } else if (state === FingerOverlapStates.end && this.wasObserverHovering(o)) {
              obj.emit(f.name + FingerOverlapEvents.hover + FingerOverlapStates.end)
              this.setObserverHover(o, false)
            }
          }
    ...
    addOverlapEmitter(thumb.setOnCollide.bind(thumb), FingerOverlapStates.start)
    addOverlapEmitter(thumb.setOnCollideActive.bind(thumb), FingerOverlapStates.continue)
    addOverlapEmitter(thumb.setOnCollideEnd.bind(thumb), FingerOverlapStates.end)
  }
  ...
  public addPinchCheck(fingerName: string, obj: GameObject, callbacks: PinchCallbacks) {
    ...
    const overlapping = (fingerName: string): boolean => {
      ...
      const fingerOverlapping: boolean = this.scene.matter.overlap(obj, [finger])
      return thumbOverlapping && fingerOverlapping
    }

    if (callbacks.startPinch != undefined) {
      obj.on(fingerName + FingerOverlapEvents.pinch + FingerOverlapStates.start, () => {
        if (overlapping(fingerName)) callbacks.startPinch?.()
      })
    }
    if (callbacks.pinched != undefined) {
      obj.on(fingerName + FingerOverlapEvents.pinch + FingerOverlapStates.continue, () => {
        if (overlapping(fingerName)) callbacks.pinched?.()
      })
    }
    if (callbacks.endPinch != undefined) {
      obj.on(fingerName + FingerOverlapEvents.pinch + FingerOverlapStates.end, () => {
        if (overlapping(fingerName)) callbacks.endPinch?.()
      })
    }
    if (callbacks.startHover != undefined) {
      obj.on(fingerName + FingerOverlapEvents.hover + FingerOverlapStates.start, () => {
        callbacks.startHover?.()
      })
    }
    if (callbacks.hovering != undefined) {
      obj.on(fingerName + FingerOverlapEvents.hover + FingerOverlapStates.continue, () => {
        callbacks.hovering?.()
      })
    }
    if (callbacks.endHover != undefined) {
      obj.on(fingerName + FingerOverlapEvents.hover + FingerOverlapStates.end, () => {
        callbacks.endHover?.()
      })
    }
  }
  ...
  public removePinchCheck(fingerName: string, obj: GameObject) {
    ...
    obj.off(fingerName + FingerOverlapEvents.pinch + FingerOverlapStates.continue)
    ...
    obj.off(fingerName + FingerOverlapEvents.hover + FingerOverlapStates.continue)
  ...
  public update() {
    ...
    const pointer = this.scene.matter.add.circle(this.scene.input.mousePointer.x, this.scene.input.mousePointer.y, 1, {
    ...
    for (const finger of this.fingers) {
      const position: Vector2 | undefined = handTracker.getLandmarkPosition(finger.landmarkIndex)
      ...
      if (!foundFinger && pointer != undefined) {
        for (const observer of this.observers) {
          ...
          observer.object.emit(finger.name + FingerOverlapEvents.pinch + FingerOverlapStates.end)
          ...
          if (!this.scene.matter.overlap(pointer, [observer.object])) {
            observer.object.emit(finger.name + FingerOverlapEvents.hover + FingerOverlapStates.end)
          }
        }
      }
    }
  ...
  calculateHandDistance(handIndex: HandIndex = 0): number {
    const pos1: Vector2 | undefined = handTracker.getLandmarkPosition(HandLandmarkIndex.INDEX_FINGER_MCP, handIndex)
    const pos2: Vector2 | undefined = handTracker.getLandmarkPosition(HandLandmarkIndex.PINKY_MCP, handIndex)
    const pos3: Vector2 | undefined = handTracker.getLandmarkPosition(HandLandmarkIndex.WRIST, handIndex)
    ...
    if (pos1 !== undefined && pos2 !== undefined) {
      ...
      distanceWidth = Math.sqrt(((pos2.x - pos1.x) / screenSize) ** 2 + ((pos2.y - pos1.y) / screenSize) ** 2)
      if (pos3 !== undefined) {
        distanceHeight = Math.sqrt(((pos3.x - pos4.x) / screenSize) ** 2 + ((pos3.y - pos4.y) / screenSize) ** 2)
      }
    }
    if (distanceWidth == 0 || distanceHeight == 0) {
      return 0
    }
    const distanceOnScreen = Math.max(distanceWidth / palmWidth, distanceHeight / palmHeight)
