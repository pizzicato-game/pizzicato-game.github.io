...
export function drawHandLandmarks(graphics: Graphics, options: LandmarkOptions, handIndex: HandIndex = 0) {
...
export function drawHandLandmarkConnections(
  ...
  handConnections.forEach(connection => {
    const pos1: Vector2 | undefined = handTracker.getLandmarkPosition(connection[0], handIndex)
    if (pos1 !== undefined) {
      const pos2: Vector2 | undefined = handTracker.getLandmarkPosition(connection[1], handIndex)
      if (pos2 !== undefined) {
        graphics.lineStyle(options.lineWidth, options.color, options.alpha)
